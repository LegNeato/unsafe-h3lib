...
unsafe extern "C" fn runTests() {
    currentTestName = b"gridPathCells_acrossMultipleFaces\0" as *const u8 as *const libc::c_char;
    let mut start: H3Index = 0x85285aa7fffffff as libc::c_long as H3Index;
    ...
    currentTestName = b"gridPathCells_pentagon\0" as *const u8 as *const libc::c_char;
    let mut start_0: H3Index = 0x820807fffffffff as libc::c_long as H3Index;
    let mut end_0: H3Index = 0x8208e7fffffffff as libc::c_long as H3Index;
    let mut size_0: int64_t = 0;
    if gridPathCellsSize(start_0, end_0, &mut size_0) != 0 {
        fprintf(
            __stderrp,
            b"%s.%s: t_assert failed at %s:%d, %s, %s\n\0" as *const u8 as *const libc::c_char,
            currentSuiteName,
    ...
    globalTestCount += 1;
    printf(b".\0" as *const u8 as *const libc::c_char);
    let mut path: *mut H3Index = calloc(
    ...
    if !(gridPathCells(start_0, end_0, path) == E_PENTAGON as libc::c_int as libc::c_uint) {
        fprintf(
            ...
            b"H3_EXPORT(gridPathCells)(start, end, path) == E_PENTAGON\0" as *const u8
                as *const libc::c_char,
            b"Line not computable due to pentagon distortion\0" as *const u8 as *const libc::c_char,
        );
        exit(1 as libc::c_int);
    }
    globalTestCount += 1;
