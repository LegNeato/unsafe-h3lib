...
extern "C" {
    ...
    fn originToDirectedEdges(origin: H3Index, edges_0: *mut H3Index) -> H3Error;
}
...
unsafe fn main_0(mut _argc: libc::c_int, mut _argv: *mut *mut libc::c_char) -> libc::c_int {
    ...
    originToDirectedEdges(hex, edges.as_mut_ptr());
    let iterations: libc::c_int = 10000 as libc::c_int;
    let mut name: *const libc::c_char =
        b"directedEdgeToBoundary\0" as *const u8 as *const libc::c_char;
    ...
    while i < iterations {
        ...
        while i_0 < 6 as libc::c_int {
            directedEdgeToBoundary(edges[i_0 as usize], &mut outBoundary);
            i_0 += 1;
        }
        i += 1;
    }
