...
#[no_mangle]
pub static mut MAX_SZ: libc::c_int = 4000000 as libc::c_int;
#[no_mangle]
pub static mut MAX_HOLES: libc::c_int = 100 as libc::c_int;
#[no_mangle]
pub unsafe extern "C" fn populateGeoLoop(
    mut g: *mut GeoLoop,
    mut data: *const uint8_t,
    mut offset: *mut size_t,
    mut size: size_t,
) -> libc::c_int {
    if size < (*offset).wrapping_add(::core::mem::size_of::<libc::c_int>() as libc::c_ulong) {
        return 1 as libc::c_int;
    }
    let mut numVerts: libc::c_int = *(data.offset(*offset as isize) as *const libc::c_int);
    *offset = (*offset).wrapping_add(::core::mem::size_of::<libc::c_int>() as libc::c_ulong);
    (*g).numVerts = numVerts;
    if size
        < (*offset).wrapping_add(
            (::core::mem::size_of::<LatLng>() as libc::c_ulong)
                .wrapping_mul(numVerts as libc::c_ulong),
        )
    {
        return 1 as libc::c_int;
    }
    (*g).verts = data.offset(*offset as isize) as *mut LatLng;
...
pub unsafe extern "C" fn run(
    mut geoPolygon: *mut GeoPolygon,
    mut flags: uint32_t,
    mut res: libc::c_int,
) {
    let mut sz: int64_t = 0;
    let mut err: H3Error = maxPolygonToCellsSize(geoPolygon, res, flags, &mut sz);
    if err == 0 && sz < MAX_SZ as libc::c_longlong {
        let mut out: *mut H3Index = calloc(
            sz as libc::c_ulong,
            ::core::mem::size_of::<H3Index>() as libc::c_ulong,
        ) as *mut H3Index;
        polygonToCells(geoPolygon, res, flags, out);
...
pub unsafe extern "C" fn LLVMFuzzerTestOneInput(
    ...
    let mut res: libc::c_int = (*args).res % (MAX_RES + 1 as libc::c_int);
    ...
    geoPolygon.holes = calloc(
        geoPolygon.numHoles as libc::c_ulong,
        ::core::mem::size_of::<GeoLoop>() as libc::c_ulong,
    ) as *mut GeoLoop;
    let mut offset: size_t = (::core::mem::size_of::<inputArgs>() as libc::c_ulong)
        .wrapping_sub(::core::mem::size_of::<[uint8_t; 1024]>() as libc::c_ulong);
    if populateGeoLoop(&mut geoPolygon.geoloop, data, &mut offset, size) != 0 {
...
unsafe fn main_0(mut argc: libc::c_int, mut argv: *mut *mut libc::c_char) -> libc::c_int {
    ...
    let mut data: [uint8_t; 1032] = [0; 1032];
    if fread(
        &mut data as *mut [uint8_t; 1032] as *mut libc::c_void,
        ::core::mem::size_of::<inputArgs>() as libc::c_ulong,
        1 as libc::c_int as libc::c_ulong,
        fp,
    ) != 1 as libc::c_int as libc::c_ulong
