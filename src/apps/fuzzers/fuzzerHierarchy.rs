...
pub struct inputArgs {
    pub index: H3Index,
    pub parentRes: libc::c_int,
    pub childRes: libc::c_int,
}
#[no_mangle]
...
pub unsafe extern "C" fn LLVMFuzzerTestOneInput(
    ...
    cellToParent((*args).index, (*args).parentRes, &mut parent);
    let mut out: H3Index = 0;
    cellToCenterChild((*args).index, (*args).childRes, &mut out);
    if (*args).childRes
        >= -(2147483647 as libc::c_int) - 1 as libc::c_int + getResolution((*args).index)
    {
        let mut resDiff: libc::c_int = (*args).childRes - getResolution((*args).index);
        if resDiff < 10 as libc::c_int {
            let mut childrenSize: int64_t = 0;
            let mut err: H3Error =
                cellToChildrenSize((*args).index, (*args).childRes, &mut childrenSize);
            if err == 0 {
                let mut children: *mut H3Index = calloc(
                    childrenSize as libc::c_ulong,
                    ::core::mem::size_of::<H3Index>() as libc::c_ulong,
                ) as *mut H3Index;
                cellToChildren((*args).index, (*args).childRes, children);
