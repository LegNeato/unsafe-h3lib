...
pub unsafe extern "C" fn LLVMFuzzerTestOneInput(
    ...
    let mut outBool: libc::c_int = 0;
    areNeighborCells((*args).index, (*args).index2, &mut outBool);
    let mut out: H3Index = 0;
    cellsToDirectedEdge((*args).index, (*args).index2, &mut out);
    isValidDirectedEdge((*args).index);
    getDirectedEdgeOrigin((*args).index, &mut out);
    getDirectedEdgeDestination((*args).index, &mut out);
    let mut out2: [H3Index; 2] = [0; 2];
    directedEdgeToCells((*args).index, out2.as_mut_ptr());
    let mut out6: [H3Index; 6] = [0; 6];
    originToDirectedEdges((*args).index, out6.as_mut_ptr());
    let mut bndry: CellBoundary = CellBoundary {
        numVerts: 0,
        verts: [LatLng { lat: 0., lng: 0. }; 10],
    };
    directedEdgeToBoundary((*args).index, &mut bndry);
    return 0 as libc::c_int;
}
