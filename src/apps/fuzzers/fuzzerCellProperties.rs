...
pub unsafe extern "C" fn LLVMFuzzerTestOneInput(
    ...
    getResolution((*args).index);
    getBaseCellNumber((*args).index);
    isValidCell((*args).index);
    isPentagon((*args).index);
    isResClassIII((*args).index);
    let mut faceCount: libc::c_int = 0;
    let mut err: H3Error = maxFaceCount((*args).index, &mut faceCount);
    if err == 0 && faceCount > 0 as libc::c_int {
        let mut out: *mut libc::c_int = calloc(
            faceCount as libc::c_ulong,
            ::core::mem::size_of::<libc::c_int>() as libc::c_ulong,
        ) as *mut libc::c_int;
        getIcosahedronFaces((*args).index, out);
...
unsafe fn main_0(mut argc: libc::c_int, mut argv: *mut *mut libc::c_char) -> libc::c_int {
    ...
    if fread(
        &mut data as *mut [uint8_t; 8] as *mut libc::c_void,
        ::core::mem::size_of::<inputArgs>() as libc::c_ulong,
        1 as libc::c_int as libc::c_ulong,
        fp,
    ) != 1 as libc::c_int as libc::c_ulong
